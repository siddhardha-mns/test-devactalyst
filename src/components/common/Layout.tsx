...
import { GlassFilter } from '../ui/filters/GlassFilter';
...
const Layout = ({ children, stars = {} }: LayoutProps) => {
  ...
  useEffect(() => {
    ...
    const onWheelGuard = (e: WheelEvent) => {
      const max = scrollLimitRef.current.max;
      if (window.scrollY >= max && e.deltaY > 0) e.preventDefault();
    };
    const onTouchStart = (e: TouchEvent) => {
      const t = e.touches?.[0];
      if (t) scrollLimitRef.current.touchStartY = t.clientY;
    };
    const onTouchMoveGuard = (e: TouchEvent) => {
      const t = e.touches?.[0];
      if (!t) return;
      const dy = scrollLimitRef.current.touchStartY - t.clientY;
      if (dy > 0 && window.scrollY >= scrollLimitRef.current.max) e.preventDefault();
    };
    ...
    <div className="min-h-screen overflow-hidden relative select-none" style={{ backgroundColor: 'var(--dc-bg)', color: 'var(--dc-text)' }}>
      ...
      <div className="fixed inset-0 pointer-events-none z-0">
        ...
        {!location.pathname.toLowerCase().startsWith('/components') && (
          <div className="absolute inset-0 cursor-spotlight ol ol-spot" />
        )}
      </div>
